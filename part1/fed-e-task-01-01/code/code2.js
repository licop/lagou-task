...
function averageDollarValue(cars) {
   return fp.flowRight(_average, fp.map(value => value.dollar_value))(cars)
}
...
function sanitizeNames(cars) {
  return fp.flowRight(fp.map(fp.toLower), fp.map(_underscore), fp.map(value => value.name))(cars)
}
